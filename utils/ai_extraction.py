...
---
## Term Normalization and Detail Grouping (CRITICAL)
...
For each `term`, if multiple similar or equivalent explanations/descriptions are found (even with minor differences in phrasing), **group and unify them into a single, deduplicated explanation**.
- If there are genuinely different functions, use cases, or responsibilities associated with the same term, include each as a separate explanation (in a list).
- Never repeat explanations that are essentially the same.
- Always summarize/merge explanations that are redundant or nearly identical, prioritizing the clearest, most context-specific description.
...
- `explanation`: a list of unique, deduplicated, and normalized sentences describing how that term is used or why it is relevant in the context of the data-related job described. All explanations must be merged or grouped to avoid any kind of redundancy, including paraphrased or synonymous descriptions. Do NOT explain terms generically — explain how they relate to the job. If a term only has one unique explanation, the list should contain only one item.
